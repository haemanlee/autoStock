...
slack = Slacker('xoxb-1598938386003-1595647391589-mqE5bSrJ8QkoUYOisS3gEyVp')
def dbgout(message):
    """인자로 받은 문자열을 파이썬 셸과 슬랙으로 동시에 출력한다."""
    print(datetime.now().strftime('[%m/%d %H:%M:%S]'), message)
    strbuf = datetime.now().strftime('[%m/%d %H:%M:%S] ') + message
    slack.chat.post_message('#auto-stock-bot', strbuf)
...
if __name__ == '__main__': 
    try:
        symbol_list = ['A219390','A267770','A337140','A277640'] # 자동매매를 워나는 종목 리스트 넣으면 됨. 
